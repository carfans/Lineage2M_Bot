...
- **🌐 多语言 CBT 特征采样点配置管理**：
  ...
  - **【🗑️ 删除此点位】**：一键从配置文件中移除。
- **动态命名弹窗管理与多维特征诊断**：
  - 全面支持**自定义命名弹窗管理**（不再局限于固定三分类），支持自由创建、重命名、编辑描述、删除任意弹窗配置（如“左上角提示”、“复活确认”、“活动公告”等）；
  - **弹窗深度链接 CBT 采样点**：支持为弹窗关联特定的 CBT 特征采样点，在识别时联合比对像素色差与几何位置，彻底杜绝误报；
  - 画面实时绘制黄色扫描框、绿色确认按钮框、红色准星点击中心与青色复选框；
  - **【💾 保存/更新】**：支持输入新名称一键新建，或修改现有弹窗的扫描区域 ROI、描述与关联 CBT 并写回 JSON；
  - **【➕ 新建】/【🗑️ 删除】**：支持一键清空重置录入，或从 JSON 中永久删除指定弹窗配置；
  - **【🖱️ 模拟关闭】**：真实测试“先勾选不再显示、后确认关闭”两步动作。
- **截图保存与本地图片离线调试**：
...
- **弹窗防御与按钮识别引擎 ([`src/game/popup_engine.c`])**：
  ...
  - **全弹窗一键自动巡检**：`l2m_detect_all_popups` 自动遍历全图已启用的全部命名弹窗配置。
